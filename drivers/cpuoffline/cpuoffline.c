		...
		struct device *dev)
{
	...
	kobj = kobject_create_and_add("cpuoffline", &dev->kobj);
...
static int cpuoffline_add_dev(struct device *dev)
{
	...
	ret = cpuoffline_add_dev_interface(partition, dev);

	return ret;
}

static int cpuoffline_remove_dev(struct device *dev)
{
...
EXPORT_SYMBOL_GPL(cpuoffline_unregister_governor);

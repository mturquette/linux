...
int tx_cmd_history_p;
int rx_cmd_history_p;
...
static void if_hsi_proto_read_done(struct hsi_device *dev, unsigned int size)
{
	...
	pr_debug("if_hsi_proto_read_done() is called for ch-> %d\n", dev->n_ch);
...
static void if_hsi_proto_write_done(struct hsi_device *dev, unsigned int size)
{
	...
	/* pr_info("if_hsi_proto_write_done() is called for ch-> %d\n", dev->n_ch);*/
	channel = &hsi_protocol_iface.channels[dev->n_ch];
	dev_dbg(&channel->dev->device, "%s, ch = %d\n", __func__, dev->n_ch);
...
int read_hsi(u8 *data, u32 ch, u32 *length)
{
	...
	/*pr_info("In read_hsi() function, Sleeping ... channel-> %d\n", ch);*/
	wait_event_interruptible(ipc_read_wait, (channel->rx_count != 0));
	/*pr_info("In read_hsi() function, Waking Up ... channel-> %d\n", ch);*/
	...
	/*pr_info("%s() read returned %d, actual_length = %d, ch-> %d\n", __func__, ret, actual_length, ch);
	pr_info("%s() sending CONN_CLOSED.\n", __func__);*/
	tmp = hsi_protocol_send_command(HSI_LL_MSG_CONN_CLOSED, ch, 0);
	/*pr_info("%s() Sending CONN_CLOSED Finished. ret = %d\n", __func__, tmp);*/

	*length = actual_length;
	channel->rx_count = 0;

	/*pr_info("%s() RETURNING TO IPC with ret = %d\n", __func__, ret);*/
	return ret;

...
/*
 * Register offsets from the hardkernel data sheet are listed in comment blocks
 * below. Those offsets must be multiplied by 4 before adding them to the base
 * address to get the right value
 */
 ...
 */
static struct meson_clk_cpu meson8b_a5_clk = {
...
static struct clk_hw_onecell_data meson8b_hw_onecell_data = {
	.hws = {
		...
		[CLKID_CPUCLK] = &meson8b_a5_clk.hw,
...
static int meson8b_clkc_probe(struct platform_device *pdev)
{
	 ...
	 */
	parent_hw = clk_hw_get_parent(&meson8b_a5_clk.hw);
	parent_clk = parent_hw->clk;
	ret = clk_notifier_register(parent_clk, &meson8b_a5_clk.clk_nb);
	if (ret) {
		pr_err("%s: failed to register clock notifier for a5_clk\n",
				__func__);
		goto iounmap;
	}
